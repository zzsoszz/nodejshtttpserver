(function($){
		...
		function PluginObject(target)
		{
				...
				self.init=function(initoptions)
				{
					...
					self.qmultiselectbox.find(".qvalueitem").each(function(){
						var itemvalue=$(this).text().trim();
						if(itemvalue!=""){
							if($.inArray(itemvalue,initvalues)){
					...
					self.popbox.on("click",function(ele){
						...
						if($(ele.target).hasClass("qvalueitem"))
						{
					...
					self.qcomfirm.on("click",function()
						{
							var values= self.qmultiselectbox.find(".qvalueitem.active").map(function(){
